...
import { excluirClienteAction } from '@/src/modules/clientes/clientes.actions';
...
export function BotaoExcluirCliente({
  ...
  const handleExcluir = async () => {
    ...
    if (resultado.sucesso) {
      ...
      setErro(resultado.mensagem || 'Erro ao excluir cliente.');
    }
    setExcluindo(false);
  };
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl space-y-4">
            ...
            <p className="text-sm text-slate-600">
              Tem certeza que deseja excluir o cliente <strong className="text-slate-900">{clienteNome}</strong>? Esta ação não pode ser desfeita.
            </p>
